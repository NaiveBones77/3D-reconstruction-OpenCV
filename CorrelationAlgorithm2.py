...
def corr2(blockSize, shift, dev = True):
    img1 = resize_img(cv2.imread('images/Aloe/view5.png'))
    img2 = resize_img(cv2.imread('images/Aloe/view0.png'))
    ...
    with Bar('Calculation', max=height-blockSize, suffix='%(percent)d%%') as bar:
        for i in range(height - blockSize):
            ...
            for j in range(width - blockSize):
                ...
                center = [int((x+y)/2) for x,y in zip(reversed(top_left), reversed(bottom_right))]
                disparity[center[0], center[1]] = bf / max_loc[0]
    return disparity

blocksize = 20
shift = int(blocksize*1.5)
...
imgorig = cv2.resize(cv2.imread('images/Aloe/view5.png'), (600,600), interpolation=cv2.INTER_CUBIC)
